...
export interface MoveProps
    extends Omit<PropsCircle, 'centerX' | 'centerY' | 'context'> {
    point: {
        start: Position
        end: Position
    }
...
export default ({ radius, fillStyle, strokeStyle, point }: MoveProps) => {
    const position: Position = { x: point.start.x, y: point.start.y }
    ...
    return ({ speed = 1, cb }: MoveCbProps) => {
        ...
        if (
            isHorizont &&
            isHorizontMoveLeft &&
            (position.x <= point.start.x || position.x >= point.end.x)
        )
            step.x *= -1
        if (
            isHorizont &&
        ...
        if (
            isVerticle &&
            isVerticleMoveTop &&
            (position.y <= point.start.y || position.y >= point.end.y)
        )
            step.y *= -1
        if (
            isVerticle &&
